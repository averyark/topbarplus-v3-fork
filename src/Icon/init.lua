...
function Icon.getIcon(iconUID)
	return Icon.iconsDictionary[iconUID]
end

function Icon.setTopbarEnabled(bool)
...
function Icon.new()
	...
	self.toggled:Connect(function(isSelected)
		...
		for childIconUID, _ in pairs(self.childIconsDict) do
			local childIcon = Icon.getIcon(childIconUID)
...
function Icon:updateParent()
	local parentIcon = Icon.getIcon(self.parentIconUID)
	if parentIcon then
...
function Icon:setFixedMenu(arrayOfIcons)
	-- A fixed menu is a menu which is permanently locked in the
...
function Icon:setIndicator(keyCode)
	-- An indicator is a direction button prompt with an
	-- image of the given keycode. This is useful for instance
	-- with controllers to show the user what button to
	-- press to highlight the topbar. You don't need
	-- to set an indicator for controllers as this is
	-- handled internally within the Gamepad module
